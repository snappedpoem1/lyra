...
class DNA:
    ...
    def trace_samples(self, track_id: str) -> List[Dict]:
        ...
        """
        logger.info(f"ðŸ§¬ DNA: Tracing samples for track {track_id}")
        ...
        try:
            ...
            logger.info(f"  â†’ {len(samples)} samples identified")
            return samples
        finally:
            conn.close()
    ...
    def register_sample(
        ...
        Returns:
            Success status
        """
        logger.info(f"ðŸ“ DNA: Registering sample [{original_artist} - {original_title}]")
        ...
        try:
            ...
            conn.commit()
            logger.info("  âœ“ Sample registered")
            return True
    ...
    def find_original_in_library(self, sample_info: Dict) -> Optional[str]:
        ...
        try:
            ...
            if result:
                logger.info(f"  âœ“ Original found in library: {result[0]}")
                return result[0]

            logger.info("  â„¹ Original not in library")
            return None
        finally:
            conn.close()
    ...
    def pivot_to_original(self, track_id: str) -> Optional[Dict]:
        ...
        """
        logger.info(f"ðŸ”„ DNA: Pivoting track {track_id} to original")
        
        samples = self.trace_samples(track_id)
        
        if not samples:
            logger.info("  â„¹ No samples registered for this track")
            return None
    ...
    def get_sampled_by(self, track_id: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"  â†’ {len(sampled_by)} tracks sample this")
            return sampled_by
        finally:
            conn.close()
    ...
    def import_whosampled_data(self, filepath: str) -> int:
        ...
        """
        logger.info(f"ðŸ“¥ DNA: Importing WhoSampled data from {filepath}")
        ...
        try:
            ...
            logger.info(f"  âœ“ Imported {imported} samples")
            return imported
