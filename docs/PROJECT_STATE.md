...
- Program state:
  ...
  - Wave 10 (MBID Identity Spine) is locally landed: SPEC-010 spec (`docs/specs/SPEC-010_MBID_IDENTITY_SPINE.md`), `oracle/enrichers/mb_identity.py` with `MBIdentityResolver.resolve_batch()` and `MBIdentityResolver.stats()`, `oracle mbid resolve` / `oracle mbid stats` CLI commands, `CreditMapper.map_batch()` column fix (was `musicbrainz_id`, now `recording_mbid`); 13 new contract tests bring the suite to 201 passing; first 200-track MBID resolve pass is running live against MusicBrainz
  - Wave 11 (Companion Pulse) is locally landed: SPEC-011 spec, `oracle/companion/pulse.py` (`CompanionPulse` class + `get_companion_pulse` singleton), `/ws/companion` SSE blueprint registered, 14 new contract tests bring the suite to 215 passing; `useCompanionStream.ts` SSE hook, `companionLines.ts` authored narrative templates, `LyraCompanion.tsx` is now event-driven (stream consumer with static fallback), `settingsStore.ts` extended with `notificationsEnabled`; renderer 26 tests passing, clean build
...
- `.venv\Scripts\python.exe -m pytest -q` -> success (`201 passed`) after Wave 10 MBID identity spine: `oracle/enrichers/mb_identity.py` (MBIdentityResolver, resolve_batch, stats), `oracle mbid resolve/stats` CLI commands, and credit_mapper.map_batch column fix (`musicbrainz_id` → `recording_mbid`)
- `.venv\Scripts\python.exe -m pytest -q` -> success (`215 passed`) after Wave 11 Companion Pulse: `oracle/companion/pulse.py` (CompanionPulse, get_companion_pulse), `/ws/companion` SSE blueprint, 14 new contract tests
- `cd desktop\renderer-app; npm run test` -> `26 passed` after Wave 11 frontend changes
- `cd desktop\renderer-app; npm run build` -> success after Wave 11 Companion Pulse frontend additions (`useCompanionStream`, `companionLines`, `LyraCompanion` stream consumer, `settingsStore.notificationsEnabled`)
- `.venv\Scripts\python.exe -m pytest -q` -> success (`226 passed`) after Wave 12 Oracle Action Breadth: `PlayerService.set_volume()`, `PlayerService.clear_queue()`, `/api/player/volume`, `/api/player/queue/clear`, 8 new oracle execute action types (`resume`, `set_volume`, `set_shuffle`, `set_repeat`, `clear_queue`, `play_artist`, `play_album`, `play_similar`), agentActionRouter 8 new cases, 11 new contract tests
...
2. Native audio (`miniaudio`) full 4-hour production soak is intentionally deferred until a later release-gate window; the 60-second parity soak with the rebuilt sidecar passes in dev-validation mode.
  - the dev data root at `%LOCALAPPDATA%\Lyra\dev` has been fully migrated with the complete library data (2,455 tracks, Chroma, logs, model cache); `LYRA_USE_LEGACY_DATA_ROOT` workaround is no longer required
  - the parity soak `-UseLegacyDataRoot` switch remains available for backward compat but is no longer needed
