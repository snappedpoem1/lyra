...
- Active: Wave 14 (Saved Playlist UI) — **completed S-20260307-18**: `"saved"` kind added to `PlaylistKind`; `mapPlaylistDetail` fixed for `{playlist, tracks}` saved shape; `SavedPlaylistsSection` + `CreatePlaylistModal` built and wired into `PlaylistsRoute`; `mappers.test.ts` (4 tests) + 4 new agentActionRouter tests; 241 Python + 34 frontend tests passing; clean build.
...
2. **Wave 15**: scope candidates — structure analysis coverage hardening (G-032), similarity graph growth (G-030), or acquisition waterfall improvements.
