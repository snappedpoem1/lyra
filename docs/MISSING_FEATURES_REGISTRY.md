...
| G-035 | Tauri sidecar packaging completeness | partial | clean-machine artifact proof passes, simulated install layout proof passes, installed-layout validation passes against rebuilt host, frozen runtime roots are hardened, and packaged runtime-root resolution now handles installed layouts more defensibly; remaining gap is a true blank-machine install and first launch | Install the Lyra installer EXE on a clean Windows VM and confirm first launch works |
| G-036 | Native audio production confidence | partial | `miniaudio` path exists with fallback but soak matrix is not complete | Validate long-session and device reliability, plus pause/seek/repeat/resume behavior |
