...
| G-036 | Native audio production confidence | partial | `miniaudio` path exists with fallback, parity hardening now performs restart recovery plus mutating soak checkpoints/logging, the 60-second parity soak with the rebuilt Wave 5+6 sidecar passes (pause/resume and seek mutations, restart recovery), and Wave 4 host modernization stayed green on Tauri 2; a `-UseLegacyDataRoot` workaround is in place for dev-mode soak validation since the dev data root has not yet been migrated with full library data; the full 4-hour long-session matrix is still deferred | Reopen the release-gate lane and validate a full 4-hour soak plus real-device pause/seek/repeat/resume reliability |
