...
| G-061 | Enrichment provenance and confidence | partial, active | Canonical enrichment adapters, cache, track-level provenance summaries, artist-level MBID-first identity summaries, shell-visible inspector provenance, first-pass discovery and generated-playlist evidence hooks now exist; saved playlist detail now shows per-track reasons, Why?/Proof toggles, degraded state chips, and dimmed not_configured rows; discover page provenance panel updated with degradedProviders and visual degraded treatment | Legacy provider evidence semantics (deeper confidence weighting, multi-source arbitration) still to be ported; broader recommendation explanation flows and explanation depth still partial | `oracle/enrichers/unified.py`, provider enrichers, `oracle/enrichers/mb_identity.py`, `oracle/explainability.py` |
