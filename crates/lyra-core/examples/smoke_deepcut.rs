...
use lyra_core::{
    db::init_database,
    deepcut::{
        build_tags, compute_acclaim, compute_popularity_percentile, get_stats, hunt_by_obscurity,
        hunt_with_taste_context, DEFAULT_LISTENER_BUCKETS,
    },
};
...
fn main() {
    ...
    assert!(
        pct_unknown <= 0.01,
        "FAIL: 0 listeners should be nearly unknown, got {}",
        pct_unknown
    );
    ...
    assert!(
        pct_mid >= 0.5,
        "FAIL: 150k listeners expected ≥ 0.5 percentile, got {}",
        pct_mid
    );
    ...
    assert_eq!(
        pct_max, 1.0,
        "FAIL: 10M listeners should be 1.0, got {}",
        pct_max
    );
    println!(
        "PASS: compute_popularity_percentile (unknown={:.2}, mid={:.2}, max={:.2})",
        pct_unknown, pct_mid, pct_max
    );

    // ── Test 2: compute_acclaim — pure math ──────────────────────────────────
    let no_data = compute_acclaim(0.0);
    assert!(
        (no_data - 0.5).abs() < 0.001,
        "FAIL: no Discogs data should yield 0.5, got {}",
        no_data
    );
    ...
    assert!(
        high >= 0.8,
        "FAIL: 4.5/5 Discogs should be ≥ 0.8, got {}",
        high
    );
    println!(
        "PASS: compute_acclaim (no_data={:.2}, high={:.2})",
        no_data, high
    );
    ...
    assert!(
        tags.contains(&"deepcut:true".to_string()),
        "FAIL: missing deepcut:true"
    );
    assert!(
        tags.contains(&"tier:hidden_gem".to_string()),
        "FAIL: missing tier:hidden_gem"
    );
    assert!(
        tags.contains(&"visibility:nearly_unknown".to_string()),
        "FAIL: missing visibility:nearly_unknown"
    );
    assert!(
        tags.contains(&"quality:acclaimed".to_string()),
        "FAIL: missing quality:acclaimed"
    );
    ...
    for t in &tracks {
        assert!(
            ...
            "FAIL: negative obscurity_score for {}",
            t.title
        );
    }
    ...
    println!(
        "PASS: obscurity — Grouper({:.3}) ≥ Lana({:.3})",
        grouper_max, lana_max
    );
    ...
    assert!(
        ambient
            .iter()
            .all(|t| t.genre.to_lowercase().contains("ambient")),
    ...
    assert!(
        !taste_results.is_empty(),
        "FAIL: taste hunt returned no results"
    );
    for t in &taste_results {
        assert!(
            t.deep_cut_rank >= 0.0,
            "FAIL: negative deep_cut_rank for {}",
            t.title
        );
        assert!(
            t.taste_alignment >= 0.0 && t.taste_alignment <= 1.0,
            "FAIL: taste_alignment {} out of [0,1] for {}",
            t.taste_alignment,
            t.title
        );
    }
    ...
    assert_eq!(
        stats.total_tracks, 5,
        "FAIL: expected 5 total tracks, got {}",
        stats.total_tracks
    );
    assert!(
        stats.median_obscurity >= 0.0,
        "FAIL: median_obscurity must be non-negative"
    );
    println!(
