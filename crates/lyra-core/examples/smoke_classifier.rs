...
fn main() {
    ...
    let cases = vec![
        Case {
            ...
            album: "Guero",
            path: "E-Pro.flac",
            want: VersionType::Original,
        },
        Case {
            title: "E-Pro (Live at Coachella)",
            album: "",
            path: "",
            want: VersionType::Live,
        },
        Case {
            title: "E-Pro (Remix)",
            album: "",
            path: "",
            want: VersionType::Remix,
        },
        Case {
            title: "E-Pro (Cover Version)",
            album: "",
            path: "",
            want: VersionType::Cover,
        },
        Case {
            title: "E-Pro (Official Video)",
            album: "",
            path: "",
            want: VersionType::Junk,
        },
        Case {
            title: "E-Pro (Nightcore)",
            album: "",
            path: "",
            want: VersionType::Special,
        },
        Case {
            title: "E-Pro (Slowed + Reverb)",
            album: "",
            path: "",
            want: VersionType::Special,
        },
    ...
    for c in &cases {
        ...
        if r.version_type != c.want {
            ...
            println!(
                "PASS: {:?} => {} ({:.2})",
                c.title, r.version_type, r.confidence
            );
        }
    }
    ...
    assert!(
        r.confidence <= 1.0 && r.confidence > 0.0,
        "FAIL: confidence out of bounds"
    );
    ...
    conn.execute("INSERT INTO artists (id, name) VALUES (1, 'Beck')", [])
        .unwrap();
    conn.execute(
        "INSERT INTO albums  (id, title, artist_id) VALUES (1, 'Guero', 1)",
        [],
    )
    .unwrap();
    ...
    assert_eq!(
        exact.len(),
        1,
        "FAIL: expected 1 exact group, got {}",
        exact.len()
    );
    ...
    assert!(
        !meta.is_empty(),
        "FAIL: expected >=1 metadata group, got {}",
        meta.len()
    );
    ...
    println!(
        "PASS: find_path_duplicates => {} group(s) (0 expected on clean schema)",
        paths.len()
    );
