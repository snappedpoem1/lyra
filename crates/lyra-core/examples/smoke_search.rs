...
fn main() {
    ...
    assert!(
        !results.is_empty(),
        "FAIL: expected results for 'karma police'"
    );
    assert!(
    ...
    println!(
        "PASS: fallback_text_search found {} result(s) for 'karma police'",
        results.len()
    );
    ...
    assert!(
        results
            .iter()
            .all(|r| r.artist.to_lowercase().contains("radiohead")),
        "FAIL: artist-only search returned non-Radiohead track"
    );
    println!(
        "PASS: artist-only search => {} Radiohead tracks",
        results.len()
    );
    ...
    assert!(
        !results.is_empty(),
        "FAIL: empty query should return all tracks (browse mode)"
    );
    println!(
        "PASS: empty query returns {} tracks (browse mode)",
        results.len()
    );

    // ── Test 4: find_remixes ──────────────────────────────────────────────────
    let remixes = find_remixes(
        &conn,
        Some("Radiohead"),
        Some("OK Computer"),
        Some("Karma Police"),
    ...
    assert!(
        !remixes.is_empty(),
        "FAIL: expected at least 1 remix result"
    );
    assert!(
    ...
    assert!(
        !results.is_empty(),
        "FAIL: hybrid_search with no filters should return results"
    );
    println!(
        "PASS: hybrid_search (no filters) => {} results",
        results.len()
    );
    ...
    println!(
        "PASS: hybrid_search artist filter => {} Radiohead tracks",
        results.len()
    );
    ...
    assert!(
        results
            .iter()
            .all(|r| r.version_type.as_deref() != Some("remix")),
        "FAIL: exclude_remix still returned a remix track"
    );
    println!(
        "PASS: hybrid_search exclude_remix => {} non-remix tracks",
        results.len()
    );
    ...
    println!(
        "PASS: dimensional range filter energy>=0.7 => {} tracks",
        results.len()
    );
