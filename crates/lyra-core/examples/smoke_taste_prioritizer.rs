...
fn main() {
    ...
    assert!(
        stats.no_taste,
        "FAIL: expected no_taste=true with empty taste_profile"
    );
    ...
    assert!(
        !stats.no_taste,
        "FAIL: taste profile seeded, should not be no_taste"
    );
    assert_eq!(
        stats.updated, 3,
        "FAIL: expected 3 updated, got {}",
        stats.updated
    );
    assert_eq!(
        stats.skipped, 1,
        "FAIL: expected 1 skipped (no artist), got {}",
        stats.skipped
    );
    println!(
        "PASS: prioritize_queue => updated={}, skipped={}",
    ...
    assert_eq!(
        scores, sorted,
        "FAIL: items not sorted by priority_score DESC"
    );
    println!(
        "PASS: batch sorted DESC: {}",
        items
            .iter()
            .map(|i| format!("{}({:.2})", i.artist, i.priority_score))
            .collect::<Vec<_>>()
            .join(", ")
    );
