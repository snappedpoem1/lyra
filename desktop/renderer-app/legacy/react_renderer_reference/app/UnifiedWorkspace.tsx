...
export function UnifiedWorkspace() {
  ...
  const refreshBootstrap = useCallback(async (interactiveRetry = false): Promise<void> => {
    ...
    try {
      ...
      // Retry bootstrap up to 4 times with increasing delay to ride out
      // the gap between the Tauri host spawning the backend process and
      // the Flask server being ready to accept connections.
      ...
      for (let attempt = 0; attempt < 4; attempt++) {
        try {
          ...
          if (attempt < 3) {
            ...
            setBootMessage(`Waiting for backend (attempt ${attempt + 2}/4)...`);
  ...
  useEffect(() => {
    ...
    void (async () => {
      stopHost = await listenHostBootStatus((payload) => {
        ...
        if (payload.ready) {
          // Host reports backend is healthy — verify with an actual API call
          // instead of trusting the event alone (avoids UI flicker).
          void refreshBootstrap(false);
        } else {
          setBackendReady(false);
        }
